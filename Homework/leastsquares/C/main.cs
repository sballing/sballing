...
public class main{
	public static void Main(){
		...
		using(var outfile = new System.IO.StreamWriter("best_fit_pm.txt")){
			for(double t=0; t<=16; t+=1.0/32){
				double res = 0;
				double res_p = 0;
				double res_m = 0;
				for(int j=0; j<funcs.Length; j++){
					double uncs_cs = Math.Sqrt(S[j,j]);
					res += cs[j]*funcs[j](t);
					res_p += (cs[j]+uncs_cs)*funcs[j](t);
					res_m += (cs[j]-uncs_cs)*funcs[j](t);
				}
				outfile.WriteLine($"{t} {Math.Exp(res)} {Math.Exp(res_p)} {Math.Exp(res_m)}");
			}
		}
